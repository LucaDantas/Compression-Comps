...
void encodeChunk(const std::vector<std::vector<int>>& inputChunk, std::vector<std::vector<int>>& outputChunk) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                result[i][j] = (double)inputChunk[i][j] - 128;
            }
        }
        ...
        for(int i = 0; i < n; i++) {
            for(int j = 0; j < n; j++) {
                outputChunk[i][j] = std::round(result[i][j]);
            }
        }
    }
    
void decodeChunk(const std::vector<std::vector<int>>& inputChunk, std::vector<std::vector<int>>& outputChunk) {
        int n = inputChunk.size();
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                result[i][j] = (double)inputChunk[i][j];
            }
        }
        ...
        for(int i = 0; i < n; i++) {
            for(int j = 0; j < n; j++) {
                outputChunk[i][j] = std::round(result[i][j]) + 128;
            }
        }
    }
...
int main() {
    ...
    // Declare and initialize the 2D vector
    std::vector<std::vector<int>> random2DVector(numRows, std::vector<int>(numCols));

    // Fill the 2D vector with random numbers
    for (int i = 0; i < numRows; ++i) {
        // Use std::generate to fill each row with random numbers
        std::generate(random2DVector[i].begin(), random2DVector[i].end(),
                      [&]() { return distribution(generator); });

    }

    std::cout << "Generated 2D Vector:" << std::endl;
    for (int i = 0; i < numRows; ++i) {
    ...
    decodeChunk(fct_test, random2DVector);
    std::cout << "FCT Reverted Chunk:" << std::endl;
    for (int i = 0; i < numRows; ++i) {
        for (int j = 0; j < numCols; ++j) {
            std::cout << random2DVector[i][j] << "\t";
        }
    ...
    for (int i = 0; i < numRows; i++) {
        test[i] = (double)random2DVector[0][i];
        std::cout << test[i] << "\t";
    }
    std::cout << std::endl;
    ForwardFCT(test);
    for (int i = 0; i < numRows; i++) {
        test[i] = (double)std::round(test[i]);
    }
    InverseFCT(test);
    std::cout << "Restored vector:" << std::endl;
    for (int i = 0; i < numRows; i++) {
        // test[i] = (double)random2DVector[0][i];
        std::cout << std::round(test[i]) << "\t";
    }
    std::cout << std::endl;
